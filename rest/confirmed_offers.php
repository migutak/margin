<?php
    ...
    ,ccypair,orderdate,buyorderamount+sellorderamount orderamount,buysell,buysellbank,if(buysell='BUY',3,-3) limitnum,currentstatus,recipient,comment, custcomment,
    ordertypefk,status,if(buysellbank='BUY' AND buyorderamount>0,'REC','PAY') recbank,if(buysellbank='BUY' AND buyorderamount>0,'PAY','REC') paybank 
    from offers left join custorders on offers.orderidfk = custorders.orderid where status = 'Accepted' and confirm = 'Confirmed' and recipient=offeredby ", $db);  
